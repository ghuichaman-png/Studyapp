...
export default function TeamAdmin() {
  ...
  if (loading) return <p className="text-slate-400">Cargando equipo…</p>
  if (error) return <div className="bg-danger/10 text-red-700 border border-danger/30 rounded-xl px-4 py-3 text-sm">{error}</div>
  if (!players.length) return <p className="text-slate-400">Aún no hay jugadores registrados.</p>

  return (
    <Card className="!p-0 overflow-hidden">
      <div className="hidden md:grid grid-cols-12 gap-2 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-400 border-b border-slate-100">
      ...
      {players.map((p) => {
        ...
        return (
          <div key={p.user_id} className="border-b border-slate-50 last:border-0">
            <div className="grid grid-cols-2 md:grid-cols-12 gap-2 px-5 py-3 items-center">
              <button onClick={() => setExpanded(open ? null : p.user_id)}
                className="md:col-span-3 font-semibold text-institutional text-left flex items-center gap-1">
                <span className={`transition-transform text-xs ${open ? 'rotate-90' : ''}`}>▸</span>
                {p.username}
              </button>
              <span className="md:col-span-2 text-right font-bold text-institutional">{p.points}</span>
              <span className="md:col-span-2 text-right text-slate-500">{pct(p.correct, p.total)}%</span>
              <span className="md:col-span-2 text-lg truncate">{p.badges.join(' ') || <span className="text-xs text-slate-300">—</span>}</span>
              <span className="md:col-span-2 text-xs text-slate-400">
                {p.last_login ? new Date(p.last_login).toLocaleDateString() : '—'}
              </span>
              <span className="md:col-span-1 text-right">
                <Btn variant="danger" onClick={() => resetProgress(p)} title="Resetear progreso">↺</Btn>
              </span>
            </div>
            {open && (
              <div className="px-5 pb-4 animate-fade-in">
                <p className="text-xs font-semibold text-slate-400 uppercase mb-2">Desglose por tema</p>
                {Object.keys(p.byTopic).length === 0 ? (
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {Object.entries(p.byTopic).map(([tid, bt]) => (
                      <div key={tid} className="bg-slate-50 rounded-lg px-3 py-2 text-sm flex justify-between">
                        <span className="text-slate-600 truncate">{topicName(tid)}</span>
                        <span className="font-semibold text-institutional">{pct(bt.correct, bt.total)}%</span>
                      </div>
                    ))}
                  </div>
                )}
